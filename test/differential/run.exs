...
defmodule Diff do
  ...
  def run_wasm(programs) do
    ...
    {json, 0} = System.cmd(node_bin(), [Path.join(@here, "driver.mjs"), progs_file, @wasm], stderr_to_stdout: true)
    File.rm(progs_file)
  ...
  defp canon({:ok, out}), do: {:ok, out |> String.replace(~r/\s+$/m, "") |> String.trim_trailing("\n")}
  ...
  defp known?(id, ledger), do: Enum.any?(ledger, &(id == &1 or String.starts_with?(id, &1 <> ":")))

  # ── run ────────────────────────────────────────────────────────────────────
  def run do
    progs = Enum.flat_map(@lenses, & &1.programs())
    unless File.exists?(@wasm), do: bail("no interpreter at #{@wasm} — run: mix wasm.build --out wasm")
    ...
    if fresh != [] do
      ...
      bail("#{length(fresh)} new divergence(s) — fix, or add to test/differential/LEDGER with a reason")
    else
